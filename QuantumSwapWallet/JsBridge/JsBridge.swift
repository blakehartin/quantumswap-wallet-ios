...
// `QuantumSwapJSBridge.java` one-to-one. Every method preserves the
// Android contract: push vs pull transport, argument encoding, and
// return shape.
// Android reference:
// app/src/main/java/com/quantumswap/app/bridge/QuantumSwapJSBridge.java
...
public final class JsBridge: @unchecked Sendable {
    ...
    /// and U+2028 / U+2029. 1:1 with `QuantumSwapJSBridge.escapeForJs`.
    ...
    // MARK: - DEX (pull-model allowlist)

    /// Allowlisted DEX bridge handlers. Mirrors Android
    /// `QuantumSwapJSBridge.DEX_METHODS`. Unknown names throw so a
    /// corrupted caller cannot turn this into arbitrary JS eval.
    private static let dexMethods: Set<String> = [
    ...
    /// (pull model). Optional `privKey` / `pubKey` are staged on the
    /// binary channel for submit handlers (`dexWalletFromBinaryKeys`
    /// in bridge.html) — never interpolated into the evaluate string.
    /// Must be called off the main thread.
    @discardableResult
    public func dexCall(method: String, payload: [String: Any],
        privKey: Data? = nil, pubKey: Data? = nil) throws -> String {
        guard Self.dexMethods.contains(method) else {
            throw JsEngineError.callFailed("Unknown DEX bridge method: \(method)")
        }
        let settle = Self.dexSubmitMethods.contains(method)
            ? Self.signingTimeoutSeconds : Self.defaultTimeoutSeconds
        // Capture binary key material for submit calls; wipe after the
        // settle wait so key bytes do not linger past the bridge hop.
        return try blockingCall(settleTimeout: settle, label: method) { cb, rid in
            if let privKey {
                try JsEngine.shared.storePendingPayloadBinary(
                    requestId: rid, key: "privKey", data: privKey)
            }
            ...
            JsEngine.shared.registerCallback(requestId: rid, callback: cb)
            try JsEngine.shared.storePendingPayload(requestId: rid, json: json)
            JsEngine.shared.evaluate("bridge.\(method)('\(rid)')")
        }
    }
...
public extension JsBridge {
    ...
    func dexCallAsync(method: String, payload: [String: Any],
        privKey: Data? = nil, pubKey: Data? = nil) async throws -> String {
        // Serialize before the Sendable hop — `[String: Any]` is not
        // Sendable under Swift concurrency.
        let payloadData = try JSONSerialization.data(
            withJSONObject: payload, options: [.sortedKeys])
        return try await withDetachedThrowing {
            guard let obj = try JSONSerialization.jsonObject(with: payloadData)
                as? [String: Any] else {
                throw JsEngineError.callFailed("DEX payload encode failed")
            }
            return try JsBridge.shared.dexCall(method: method, payload: obj,
