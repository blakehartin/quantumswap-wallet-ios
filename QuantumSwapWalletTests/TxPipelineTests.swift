...
final class DexAllowlistTests: XCTestCase {

    func testAllowlistIncludesGasEstimateAndTokenCreate() {
        XCTAssertTrue(JsBridge.isAllowlistedDexMethod("dexEstimateGas"))
        XCTAssertTrue(JsBridge.isAllowlistedDexMethod("tokensSubmitCreate"))
        XCTAssertTrue(JsBridge.isAllowlistedDexMethod("swapSubmitSwap"))
    }

    func testAllowlistExcludesLegacyEstimators() {
        XCTAssertFalse(JsBridge.isAllowlistedDexMethod("swapEstimateGas"))
        XCTAssertFalse(JsBridge.isAllowlistedDexMethod("swapEstimateApproveGas"))
        XCTAssertFalse(JsBridge.isAllowlistedDexMethod("tokensEstimateDeployGas"))
    }
}

final class RecognizedTokensListTests: XCTestCase {

    func testSevenRecognizedTokensInDesktopOrder() {
        XCTAssertEqual(RecognizedTokens.all.count, 7)
        XCTAssertEqual(RecognizedTokens.listed.map { $0.symbol }, ["HSN", "Y2Q", "Lio", "tig", "cat", "pant", "WQ"])
        for entry in RecognizedTokens.listed {
            XCTAssertTrue(RecognizedTokens.isRecognized(entry.address))
        }
        XCTAssertEqual(RecognizedTokens.displaySymbol(RecognizedTokens.lion.uppercased()), "Lio")
        XCTAssertNil(RecognizedTokens.displaySymbol("0x00"))
    }
}
